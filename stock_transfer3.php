...
if (isset($_POST['transfer'])) {
    ...
    if ($existingProductResult->num_rows > 0) {
        ...
        $newK1Quantity = $existingProductData['wowmoto'] - 1;
    
        $updateQuantity = $conn->prepare("UPDATE products SET $destination = ?, wowmoto = ? WHERE id = ?");
        ...
        if ($updateQuantity->affected_rows > 0) {
            ...
            header("Location: stock_transfer3.php");
            exit();
        } else {
            // Error, redirect back to the product management page with an error message
            header("Location: stock_transfer3.php?error=Failed to add the product. Please try again.");
            exit();
        }
    } else {
...
<body>
    <div class="container">
        ...
        <div class="signout-container">
            <form action="stock_transfer3.php" method="post">
                <div class="form-group">
                    <button type="button" onclick="window.location.href='restricted_inventory3.php'">Back</button>
                </div>
