...
class NEWTCPDF extends TCPDF {
    ...
    public function Header() {
        // Logo
        $image_file = 'phplogin/garios_logo.png';
        $this->Image($image_file, 10, 10, 15, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        // Set font
        $this->SetFont('helvetica', 'B', 20);
        // Title
        $this->Cell(0, 15, 'Garios Total Sales', 0, false, 'C', 0, '', 0, false, 'M', 'M');
    }
...
$summarySql = "SELECT sku, brand, model, size, price, quantity FROM sold_items2 GROUP BY sku";
$summaryResult = $conn->query($summarySql);

if ($summaryResult->num_rows > 0) {
    // Create a table to display the sold itemsa
    $html = '<h2>Garios Total Sales</h2>'; // Add the heading
...
$pdf->Output('sold_items_garios.pdf', 'D');
?>
