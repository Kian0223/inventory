...
if (isset($_POST['transfer'])) {
    ...
    if ($existingProductResult->num_rows > 0) {
        ...
        $newSelectedQuantity = $existingProductData[$destination] + 1;
        $newK1Quantity = $existingProductData['bodega_home'] - 1;
    
        $updateQuantity = $conn->prepare("UPDATE products SET $destination = ?, bodega_home = ? WHERE id = ?");
        $updateQuantity->bind_param("iii", $newSelectedQuantity, $newK1Quantity, $existingProductId);
        $updateQuantity->execute();

        if ($updateQuantity->affected_rows > 0) {
            // Success, redirect back to the product management page
            header("Location: stock_transfer4.php");
            exit();
        } else {
            // Error, redirect back to the product management page with an error message
            header("Location: stock_transfer4.php?error=Failed to add the product. Please try again.");
            exit();
        }
    } else {
...
<body>
    <div class="container">
        ...
        <div class="signout-container">
            <form action="stock_transfer4.php" method="post">
                <div class="form-group">
                    <button type="button" onclick="window.location.href='restricted_inventory4.php'">Back</button>
                </div>
