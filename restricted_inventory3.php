...
<script>
    function downloadPDF() {
        ...
        window.location.href = "generate_pdf3.php";
    }
</script>
    <div class="container">
        <h3 style="text-align: right; font-weight: bold; font-size: 20px;">Hello Wowmoto!</h3>
        <div class="signout-container">
            <form action="employee_login3.php" method="get">
                <button type="submit" name="signout">Sign Out</button>
            </form>
        </div>
        <h2>Product Management (Employee)</h2>
        <form action="restricted_inventory3.php" method="post">
            ...
            <div class="form-group">
                <button type="button" onclick="window.location.href='stock_transfer3.php'">Stock Transfer</button>
            </div>
        </form>
        <div class="summary-container">
            <h3>Sold Items Summary (wowmoto)</h3>
            ...
            <table>
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    if (isset($_POST['sell'])) {
                        ...
                        if ($result->num_rows > 0) {
                            // The SKU exists, subtract 1 from the wowmoto column
                            $updateSql = "UPDATE products SET wowmoto = wowmoto - 1 WHERE sku = '$sku'";
                            $updateResult = $conn->query($updateSql);

                            if ($updateResult === true) {
                                // Check if the SKU already exists in the summary table
                                $checkSql = "SELECT * FROM sold_items3 WHERE sku = '$sku'";
                                $checkResult = $conn->query($checkSql);

                                if ($checkResult->num_rows > 0) {
                                    // SKU already exists, update the quantity for the existing row
                                    $updateQuantitySql = "UPDATE sold_items3 SET quantity = quantity + 1 WHERE sku = '$sku'";
                                    ...
                                    $productDetailsSql = "SELECT * FROM products WHERE sku = '$sku'";
                                    ...
                                    if ($productDetailsResult->num_rows > 0) {
                                        ...
                                        $insertSummarySql = "INSERT INTO sold_items3 (sku, brand, model, size, price, quantity) VALUES ('$sku', '$brand', '$model', '$size', '$price', 1)";
                                ...
                                header("Location: restricted_inventory3.php");
                                exit();
                            } else {
                                // Handle the error if the update fails
                            ...
                            $insertSummarySql = "INSERT INTO sold_items3 (sku, brand, model, size, price, quantity) VALUES ('$sku', 'N/A', 'N/A', 'N/A', 'N/A', 1)";
                            $insertSummaryResult = $conn->query($insertSummarySql);
                ...
                $summarySql = "SELECT sku, brand, model, size, price, quantity FROM sold_items3";
