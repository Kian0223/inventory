...
class NEWTCPDF extends TCPDF {
    ...
    public function Header() {
        // Logo
        $image_file = 'phplogin/k1_logo.png';
        $this->Image($image_file, 10, 10, 15, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        // Set font
        $this->SetFont('helvetica', 'B', 20);
        // Title
        $this->Cell(0, 15, 'K1 Total Sales', 0, false, 'C', 0, '', 0, false, 'M', 'M');
    }
...
$summarySql = "SELECT sku, brand, model, size, price, quantity FROM sold_items GROUP BY sku";
$summaryResult = $conn->query($summarySql);

if ($summaryResult->num_rows > 0) {
    // Create a table to display the sold items
    $html = '<h2>K1 Total Sales</h2>'; // Add the heading
...
$pdf->Output('sold_items_K1.pdf', 'D');
?>
