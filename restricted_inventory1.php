...
<script>
    function downloadPDF() {
        ...
        window.location.href = "generate_pdf1.php";
    }
</script>
    <div class="container">
        <h3 style="text-align: right; font-weight: bold; font-size: 20px;">Hello Kishii!</h3>
        <div class="signout-container">
            <form action="employee_login1.php" method="get">
                <button type="submit" name="signout">Sign Out</button>
            </form>
        </div>
        <h2>Product Management (Employee)</h2>
        <form action="restricted_inventory1.php" method="post">
            ...
            <div class="form-group">
                <button type="button" onclick="window.location.href='stock_transfer1.php'">Stock Transfer</button>
            </div>
        </form>

        <div class="summary-container">
            <h3>Sold Items Summary (Kishii)</h3>
            ...
            <table>
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    if (isset($_POST['sell'])) {
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            // The SKU exists, subtract 1 from the kishii column
                            $updateSql = "UPDATE products SET kishii = kishii - 1 WHERE sku = '$sku'";
                            $updateResult = $conn->query($updateSql);

                            if ($updateResult === true) {
                                // Check if the SKU already exists in the summary table
                                $checkSql = "SELECT * FROM sold_items1 WHERE sku = '$sku'";
                                $checkResult = $conn->query($checkSql);

                                if ($checkResult->num_rows > 0) {
                                    // SKU already exists, update the quantity for the existing row
                                    $updateQuantitySql = "UPDATE sold_items1 SET quantity = quantity + 1 WHERE sku = '$sku'";
                                    ...
                                    $insertSummarySql = "INSERT INTO sold_items1 (sku, brand, model, size, price, quantity) VALUES ('$row[sku]', '$row[brand]', '$row[model]', '$row[size]', '$row[price]', 1)";
                            ...
                            $insertSummarySql = "INSERT INTO sold_items1 (sku, brand, model, size, price, quantity) VALUES ('$sku', 'N/A', 'N/A', 'N/A', 'N/A', 1)";
                            $insertSummaryResult = $conn->query($insertSummarySql);
                ...
                $summarySql = "SELECT sku, brand, model, size, price, quantity FROM sold_items1";
